...
export class RemoveController implements Interface.RemoveControllerInterface {
  ...
  async removeController(
    ...
    if (showConfirmation) {
      await vscode.window
        .showWarningMessage(`Remove ${selectedController.label}?`, "Yes", "No")
        .then((value) => {
  ...
  async editSettings(controller: ControllerItem | undefined) {
    ...
    if (controller === undefined) {
      ...
      await EditSettingsFunctionality.editSetting(
        ...
        settingAdapter[controller.setting as keyof typeof settingAdapter],
      );
    }
    ...
    if (!element) {
      ...
      if (element instanceof Controller) {
        ...
        if (settings.connection === "ethernet") {
          settingArray.push(
            new ControllerItem(element.controllerId, setting.ip, settings.ip),
          );
        }
        settingArray.push(
          new ControllerItem(element.controllerId, setting.port, settings.port),
        );
        settingArray.push(
          new ControllerItem(element.controllerId, setting.user, settings.user),
        );
        settingArray.push(
          new ControllerItem(
            element.controllerId,
...
export class UploadFunctionality {
  ...
  private async deactivateCodeSys3(id: number) {
    ...
    await connectionManager
      .executeCommand(id, "/etc/config-tools/config_runtime runtime-version=0")
      .then(() => {
  ...
  private async updateContainer(id: number) {
    ...
    try {
      ...
      if (autoupdate === "off") {
        await vscode.window
          .showWarningMessage(`Update Container on ${conName}?`, "Yes", "No")
          .then((value) => {
            if (value === "No") return;
          });
      }
      ...
      fs.unlink(`%tmp%/image.tar`, (err) => {
        if (err)
          console.debug("Error removing image.tar from the temp folder.");
  ...
  private getImageHash(layers: string[]): string {
    let hash = crypto.createHash("md5").update(layers.toString()).digest("hex");
