...
export class webview_IOCheck {
    ...
    async update_io_check() {
        ...
        if (this.ioCheckPanel) {
            await this.cc100Engine.analog_calib_data().then((tmp: string) => {
            ...
            this.ioCheckPanel.webview.onDidReceiveMessage(
                async message => {
                    ...
                    switch (message.command) {
                        ...
                        case 'readData': {
                            await this.cc100Engine.read_CC100().then((data: any) => {
                                /**
                                 * `[0]` => digital inputs
                                 * `[1]` => digital outputs
                                 * `[2]` => analog input 1
                        ...
                        case 'digitalWrite': {
                            ...
                            await this.cc100Engine.digital_write(value).then(() => {
                                this.ioCheckPanel?.webview.postMessage({
                                    command: 'buttonClick'
                                })
                            });
                        ...
                        case 'analogWrite': {
                            ...
                            await this.cc100Engine.analog_write(message.pin, value).then(() => {
                                this.ioCheckPanel?.webview.postMessage({
                                    command: 'buttonClick',
                                    pin: message.pin
                                })
                        ...
                        case 'serialWrite': {
                            ...
                            await this.cc100Engine.serial_write(text).then(() => {
                                this.ioCheckPanel?.webview.postMessage({
                                    command: 'serialWrite',
                                    text: text
                                })
    ...
    private async start_event_for_serial_communication() {
        ...
        await ssh.ssh2_connect()
        await this.cc100Engine.serial_read((rxData: Buffer) => {
    ...
    private async start_event_for_switch() {
        this.cc100Engine.read_switch_status((switch1: Buffer[]) => {
