...
import { cc100Engine } from '../cc100Engine';
...
export class custom_webview_provider_menu implements vscode.WebviewViewProvider {
    ...
    private cc100engine = new cc100Engine();
    private pythonWorkspace = new pythonWorkspace();
    ...
    **/
    private async btn_delete_clicked() {
        ...
        if (!ws_path.startsWith("Error")) {
            ...
            if (!result.toString().startsWith('Error')) {
                console.log("Connection successful");
                await this.pythonWorkspace.kill_all_python_scripts();
                ...
                await this.cc100engine.digital_write(0);
                await this.cc100engine.analog_write(1, 0);
                await this.cc100engine.analog_write(2, 0);
                await this.cc100engine.turn_off_run_led();
                await this.ssh.kill_all_tails();
                await this.ssh.start_codesys_runtime();
                await this.ssh.disconnect_ssh();
   ...
   */
    private async run_application(debug: boolean, progress: any): Promise<void> {
        ...
        if (!ws_path.startsWith("Error")) {
            ...
            await this.ssh.ssh_connection_with_key().then(async () => {
                await this.cc100engine.setup_serial_interface();
                await this.ssh.disconnect_ssh();
            })
    ...
    private async check_for_latest_python_version(): Promise<Boolean | void> {
        ...
        let result_get_python_version = await this.pythonWorkspace.get_python_version();
    ...
    private async update_python_version(): Promise<boolean> {
        ...
        await this.ssh.ssh_connection_with_key().then(async () => {
            let result_install_package = await this.pythonWorkspace.install_package(this.dest_path_to_ipk);
    ...
    private async start_application(): Promise<void> {
        await this.ssh.ssh_connection_with_key().then(async () => {
            ...
            let result_kill_all_python_scripts = await this.pythonWorkspace.kill_all_python_scripts();
            ...
            let result_start_python_script = await this.pythonWorkspace.start_python_script(this.dest_path + "/lib/runtimeCC.py");
    ...
    private async create_bootapplication(): Promise<boolean> {
        ...
        if (!result_upload_bootapplication.startsWith("Error")) {
            ...
            let result_make_file_executable = await this.pythonWorkspace.make_file_to_executable_file(this.filename_on_startup);
    ...
    private async create_log_listener() {
        ...
        if (!result.includes("Error")) {
            return new Promise(async (resolve, reject) => {
                resolve(null)
                await this.cc100engine.read_log(this.write_log.bind(this.write_log))
            })
        }
    }
